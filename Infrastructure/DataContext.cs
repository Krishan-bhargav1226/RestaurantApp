...
namespace Infrastructure
{
    public class DataContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<TableStatusHistory>().HasOne<User>().WithMany().HasForeignKey(x => x.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
            ...
            modelBuilder.Entity<Order>().HasOne<Customer>().WithMany().HasForeignKey(x => x.CustomerId).OnDelete(DeleteBehavior.Restrict);
            modelBuilder.Entity<Order>().HasOne<CustomerAddress>().WithMany().HasForeignKey(x => x.AddressId).OnDelete(DeleteBehavior.Restrict);
            modelBuilder.Entity<Order>().HasOne<User>().WithMany().HasForeignKey(x => x.CreatedByUserId).OnDelete(DeleteBehavior.Restrict);
            ...
            modelBuilder.Entity<User>().HasOne<Branch>().WithMany().HasForeignKey(x => x.BranchId).OnDelete(DeleteBehavior.Restrict);
            modelBuilder.Entity<CustomerAddress>().HasOne<Customer>().WithMany(x => x.Addresses).HasForeignKey(x => x.CustomerId).OnDelete(DeleteBehavior.Cascade);
            modelBuilder.Entity<User>().HasIndex(x => x.Email).IsUnique().HasFilter("[IsDeleted] = 0");
            modelBuilder.Entity<User>().HasIndex(x => x.Phone).IsUnique().HasFilter("[IsDeleted] = 0");
            modelBuilder.Entity<Customer>().HasIndex(x => x.Email).IsUnique().HasFilter("[IsDeleted] = 0");
            modelBuilder.Entity<Customer>().HasIndex(x => x.Phone).IsUnique().HasFilter("[IsDeleted] = 0");
            modelBuilder.Entity<CustomerAddress>().HasIndex(x => new { x.CustomerId, x.Label }).IsUnique().HasFilter("[IsDeleted] = 0");
