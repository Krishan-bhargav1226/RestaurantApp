...
public class TableSessionRepository : ITableSessionRepository
{
    ...
    public TableSessionRepository(DataContext context)
    {
        _context = context;
    }
    ...
    public async Task<List<TableSession>> GetAllAsync()
    {
        return await _context.TableSessions.ToListAsync();
    }

    public async Task<TableSession?> GetByIdAsync(int id)
    {
