...
public class TableSessionApplication : ITableSessionApplication
{
    ...
    public TableSessionApplication(ITableSessionRepository tableSessionRepository, IMapper mapper)
    {
        _tableSessionRepository = tableSessionRepository;
        _mapper = mapper;
    }

    public async Task<TableSessionResponseDto> CreateAsync(CreateUpdateTableSessionDto input)
    {
        var session = _mapper.Map<TableSession>(input);
        session.StartTime = input.StartTime ?? DateTime.UtcNow;
        session.IsActive = true;
        session.IsDeleted = false;
        return _mapper.Map<TableSessionResponseDto>(await _tableSessionRepository.CreateAsync(session));
    }
    ...
    public async Task<TableSessionResponseDto?> GetByIdAsync(int id)
    {
        ...
        return session == null ? null : _mapper.Map<TableSessionResponseDto>(session);
    }

    public async Task<TableSessionResponseDto?> UpdateAsync(int id, CreateUpdateTableSessionDto input)
    {
        var session = await _tableSessionRepository.GetByIdAsync(id);
        if (session == null)
            return null;
        ...
        return _mapper.Map<TableSessionResponseDto>(await _tableSessionRepository.UpdateAsync(session));
    }
